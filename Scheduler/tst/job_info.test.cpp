...
namespace {
	...
	TEST(JobFromString, AllZeroes) {
		...
		job_info job = job_from_string("JOBN 0 0 0 0 0 0");
	...
	TEST(JobFromString, StandardExample0) {
		...
		job_info job = job_from_string("JOBN 83 0 1566 1 200 300");
		EXPECT_EQ(job.submit_time, expected.submit_time);
		EXPECT_EQ(job.id, expected.id);
		EXPECT_EQ(job.est_runtime, expected.est_runtime);
		EXPECT_EQ(job.req_resc, expected.req_resc);
	...
	TEST(JobFromString, StandardExample1) {
		...
		job_info job = job_from_string("JOBN 290 1 7 1 400 1200");
		EXPECT_EQ(job.submit_time, expected.submit_time);
		EXPECT_EQ(job.id, expected.id);
		EXPECT_EQ(job.est_runtime, expected.est_runtime);
		EXPECT_EQ(job.req_resc, expected.req_resc);
